...
use App\Services\BuscaLivreAtoService;
use App\Services\BuscaPalavraAtoService;
use App\Services\BuscaPalavraExclusaoService;
...
class AtoPublicoController extends Controller
{
    ...
    public function buscaLivre(Request $request, Ato $ato)
    {
        try {
            ...
            if ($request->palavra != null && $request->exclusao != null){
                $atos = BuscaLivreAtoService::processarAtos($request->palavra, $request->exclusao);
            }
            else{
                if ($request->palavra != null){
                   $atos = BuscaPalavraAtoService::processarAtoPalavra($request->palavra);
                }

                if ($request->exclusao != null){
                    $atos = BuscaPalavraExclusaoService::processarAtosComExclusao($request->exclusao);
                }
            }
