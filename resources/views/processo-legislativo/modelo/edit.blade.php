...
@section('content')
    ...
    <h1 class="h3 mb-3"><span class="caminho">Processo Legislativo > </span>Alteração de Modelo</h1>
                ...
                <form action="{{ route('proposicao.modelo.update', $modelo_proposicao->id) }}" id="form" method="POST" class="form_prevent_multiple_submits">
                    ...
                    <div class="row">
                        <div class="col-md-12 mb-3">
                            ...
                            <textarea name="conteudo" class="form-control @error('conteudo') is-invalid @enderror" cols="30" rows="15" id="conteudo">{{ $modelo_proposicao->conteudo }}</textarea>
                            @error('conteudo')
                                <div class="invalid-feedback">{{ $message }}</div><br>
                            @enderror
                        </div>
