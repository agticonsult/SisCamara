                                ...
                                @foreach ($gestoesAdm as $gestaoAdm)
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <button type="button" class="btn btn-danger m-1" data-toggle="modal" data-target="#exampleModalExcluir{{ $gestaoAdm->id }}"><i class="fas fa-trash"></i></button>
                                        </td>
                                    </tr>
                                    <div class="modal fade" id="exampleModalExcluir{{ $gestaoAdm->id }}"
                                        tabindex="-1" role="dialog" aria-labelledby="exampleModalLabelExcluir"
                                        aria-hidden="true">
                                        <div class="modal-dialog" role="document">
                                            <div class="modal-content">
                                                <form method="POST" class="form_prevent_multiple_submits" action="{{ route('configuracao.gestao_administrativa.destroy', $gestaoAdm->id) }}">
                                                    @csrf
                                                    @method('POST')
                                                    <div class="modal-header btn-danger">
                                                        <h5 class="modal-title text-center" id="exampleModalLabelExcluir">
                                                            Excluir: <strong>{{ $gestaoAdm->id_departamento != null ? $gestaoAdm->departamento->descricao : 'não informado' }}</strong>
                                                        </h5>
                                                    </div>
